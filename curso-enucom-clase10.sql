...
-- ejercicio sql
--1 AMERICA 2ASIA 3 OCE4ANI 4 EUROPA 5 AFRICA
--1 LOS INNER JOIN DE GENERO Y PAIS + SUS CASE PLATAFORMA Y CONTINENTE
SELECT
S.*, 
G.NOMBRE,
P.NOMBRE,
(
    ...
    WHEN 1 THEN 'NETFLIX'
    WHEN 2 THEN 'HBO'
    WHEN 3 THEN 'DISNEY PLUS'
    WHEN 4 THEN 'TV'
    WHEN 5 THEN 'AMAZON PRIME'
    WHEN 6 THEN 'OTROS'
    ELSE '***' END
) PLATAFORMA, 
(
...
FROM SERIE18 S
INNER JOIN PAIS18S P
ON P.ID_PAIS = S.PAIS_ID
INNER JOIN GENERO18S G
ON G.ID_GENERO = S.GENERO_ID;

--2 CONTAR CUANTAS SERIES TENGO DE CADA PLATAFORMA
SELECT
(
    CASE PLATAFORMA
    WHEN 1 THEN 'NETFLIX'
...
COUNT(PLATAFORMA) TOTAL
FROM SERIE18 
GROUP BY PLATAFORMA;

--3 MOSTAR LOS REGISTROS ORDENADOS POR SU PLATAFORMA Y SU FECHA DE ESTRENO, PONER EL NOMBRE EN MAYUSCULAS Y CONCATENAR 
-- AL CAMPO COSTO EL SIGNO DE $
SELECT
UPPER(NOMBRE) NOMBRE,
TO_CHAR(FECHA_REG, 'DD-MM-YYYY') AS FECHA_ESTRENO,
(
    CASE PLATAFORMA
    WHEN 1 THEN 'NETFLIX'
    WHEN 2 THEN 'HBO'
...
CONCAT('$ ',COSTO) COSTO
FROM SERIE18 S
ORDER BY PLATAFORMA2, FECHA_REG;

--4 OBTENER LA SERIE MAS CARA
SELECT
UPPER(NOMBRE) NOMBRE,
TO_CHAR(FECHA_REG, 'DD-MM-YYYY') AS FECHA_ESTRENO,
(
...
CONCAT('$ ',COSTO) COSTO
FROM SERIE18 S
WHERE COSTO = (SELECT MAX(COSTO) FROM SERIE18);


--5 OBTENER LA SERIE CON MENOS CAPITULOS
SELECT
UPPER(NOMBRE) NOMBRE,
CAPITULOS,
TO_CHAR(FECHA_REG, 'DD-MM-YYYY') AS FECHA_ESTRENO,
...
CONCAT('$ ',COSTO) COSTO
FROM SERIE18 S
WHERE CAPITULOS = (SELECT MIN(CAPITULOS) FROM SERIE18)
